...
heading tree as an indented list; the up/down arrows move a heading (and everything nested under it) among
its **same-parent siblings only**, so nesting is always preserved.
...
3. In the dialog, use the up/down arrows:
   ...
   - Or reorder the nested siblings `Alpha detail` and `More Alpha detail` under `Alpha` - they only
     swap with each other, never leaving `Alpha`.
4. Click **Reorder**. The sections are rewritten in the new order; any content before the first heading
   (the preamble) stays put.
