...
- `Reorder child folders...`
  - reorders what is **inside** the folder you right-clicked. From the command palette it uses your
    ...
    [29 Create folder with notes](<./29 Create folder with notes.md>) does.
...
[include/exclude paths](<./21 Block commands on excluded paths.md>) are left out of the dialog
...
- **Folder note title template** (default `{{folderName}}`, under `Folder note`)
  - the `title` a reorder writes. `{{folderName}}` is the new name **with** its number,
    ...
    shared with [31 Rename folder](<./31 Rename folder.md>), because both write the same property of
    the same note.
