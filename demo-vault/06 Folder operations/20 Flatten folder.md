...
3. Read the confirmation dialog: it lists every item that is about to move, and shows the
   ...
   Choose `Flatten`.
4. Watch [Note one](<../Materials/20 Flatten folder/Flatten example/Note one.md>), [Note two](<../Materials/20 Flatten folder/Flatten example/Note two.md>), and the `Nested` subfolder pop up one level (into the vault
   root), while `Flatten example` is left behind, now empty.

The link from [Note one](<../Materials/20 Flatten folder/Flatten example/Note one.md>) to [Note two](<../Materials/20 Flatten folder/Flatten example/Note two.md>) keeps resolving after the move, and `Nested` travels
...
- `Flatten folder (child folders only)...`
  - only `Nested` moves. [Note one](<../Materials/20 Flatten folder/Flatten example/Note one.md>) and [Note two](<../Materials/20 Flatten folder/Flatten example/Note two.md>) stay
...
- `Flatten folder recursively (all folders at any depth)...`
  - `Nested` **and** its own `Deeper` subfolder both land beside `Flatten example`, so a whole tree of
    ...
    [Deep note](<../Materials/20 Flatten folder/Flatten example/Nested/Deep note.md>) stays in `Nested`,
    [Deepest note](<../Materials/20 Flatten folder/Flatten example/Nested/Deeper/Deepest note.md>) in `Deeper`.
...
its own ([Deep note](<../Materials/20 Flatten folder/Flatten example/Nested/Deep note.md>)), which is what keeps the child-folders-only command there; on a folder holding
