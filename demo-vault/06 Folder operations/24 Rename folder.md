...
The new name is refused if it would be empty or if it still contains invalid characters (with **Should replace invalid title characters** off), exactly as when creating a folder. If a sibling is already called that, the name is de-duplicated into `Beta 1` and the properties describe the folder that actually exists. To keep a folder from being renamed, list it in **Rename command exclude paths** — see [26 Block commands on excluded paths](<../08 Include and exclude/26 Block commands on excluded paths.md>) — and the vault root is never offered — it has no name of its own to change.
