...
- `{{content}}`
  - the extracted or merged text (**required** in every non-empty template).
- `{{fromTitle}}` / `{{fromPath}}`
  - the source note's title / path.
- `{{newTitle}}` / `{{newPath}}`
...
- `{{safeFolderName}}`
  - its name without its number.
- `{{index}}`
  - the number itself (empty when it has none; `{{index:000}}` zero-pads).
- `{{parentFolderPath}}`
