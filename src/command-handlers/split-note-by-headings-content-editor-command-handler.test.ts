...
describe('SplitNoteByHeadingsContentEditorCommandHandler', () => {
  ...
  it('should increment headingIndex when shouldKeepHeadingsWhenSplittingContent is true', async () => {
    ...
    mockGetCacheSafe.mockResolvedValueOnce(strictProxy<CachedMetadataEx>({ headings: [heading] }));
...
describe('SplitNoteByHeadingsContentEditorCommandHandler viewport menu placement', () => {
  it('should stay off the readable-line-length margin while the category is placed in the editor menu', () => {
    const handler = toTestable(new SplitNoteByHeadingsContentEditorCommandHandler(createMockParams(2)));
