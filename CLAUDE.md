...
- **`src/`** — plugin source:
  ...
  - `command-handlers/` — one class per command (merge file/folder, swap file/folder, extract before/after cursor/current selection/this heading, split-by-headings), each extending an `obsidian-dev-utils` command-handler base. The "smart cut & paste" feature adds `mark-selection-to-move`, `move-marked-selection-here`, and the top/bottom variants, plus `cancel-move` and `unlock-active-note` (both `GlobalCommandHandler`s). `mark-selection-to-move` holds a `shouldBlockMutations` lock while the mark is pending: by default a `mode: 'file'` lock on the source note only; when the `shouldLockAllNotesWhenMarkingSelection` setting is on, a `mode: 'subtree'` lock on the vault root (`app.vault.getRoot().path`, i.e. `/`, which covers every path via `isChild`) so every note is read-only + mutation-blocked until the move completes or is cancelled. Either way it is one `Disposable` held in `MoveSelectionBuffer` and released via `clear()`. The mark handler wires `abortController.signal` → `MoveSelectionBuffer.clear()` (identity-guarded so a stale controller can't cancel a later mark), so **aborting the lock cancels the entire move** (releases the lock, drops the mark, hides the notice) — `requestUnlock`/abort alone does NOT release a lock (the owner must observe it). `UnlockActiveNoteCommandHandler` (`command-handlers/unlock-active-note-command-handler.ts`) is available whenever the active note is locked — directly (`isResourceLockedForPath`, e.g. a merge/split operation or the source-only mark lock) or, for a pending mark, via the all-notes root subtree lock (`resourceLockComponent.isMutationBlockedByAncestorForPath`). On execute it `requestResourceUnlockForPath`s a directly-locked note (which for a mark aborts its controller → cancel), else aborts the mark's controller directly. The lock indicator's right-click unlock hits the same abort path, so it also cancels the whole move. The move-into-active-note commands share `MoveMarkedSelectionEditorCommandHandlerBase` (validation, mtime guard, buffer clear, `SplitComposer` handoff); subclasses only supply the insert point via `resolveInsertion()` and, optionally, override `resolveOptions()`. `MoveMarkedSelectionHereEditorCommandHandler` (cursor offset; registered twice via an `isAdvanced` flag for the default and advanced-options commands) keeps the paste-options modal; `MoveMarkedSelectionToEdgeEditorCommandHandler` (registered once per `InsertMode` → `move-marked-selection-to-top-of-file` / `move-marked-selection-to-bottom-of-file`, no default hotkeys) returns a `null` cursor offset so the composer derives top/bottom from `insertMode`. The move token is created by the shared `createMoveToken()` (`src/move-token.ts`).
