...
- **Should convert folders to headings when merging a folder** mirrors the folder hierarchy as headings. A direct sub-folder becomes `# Name`, its own child `## Name`, and so on (capped at `######`); notes directly inside the merged folder get no heading, since the merged note already stands for that folder. Each merged note's own headings are demoted to match, so the outline stays well-formed. This is the exact opposite of `Split note by headings recursively...`, which turns a heading hierarchy into a folder tree — split a note into a tree and merge it back and the levels agree.
